...
# -> Python2d에서 Python3의 기능 하나를 가져오는 구문, print()를 사용할 수 있게 호환.
...
try:
    ...
    pass
# -> sys.path.append() : 시스템 경로에 특정 패키지 경로를 추가
# -> glob.glob() : 지정된 패턴과 일치하는 파일의 리스트를 반환
# -> sys.version_info.major, sys.version_info.minor : 파이썬 버전값. ex) python 3.8 : 3, 8
# -> 'win-amd64' if os.name == 'nt' else 'linux-x86_64' : 윈도우OS인지 리눅스OS인지 결정
# -> 총정리 : 파이썬 버전과 OS정보를 이용하여, 시스템 경로에 적절한 패키지 경로를 추가
...
import carla # -> carla 시뮬레이션을 위한 PythonAPI 제공.

from carla import ColorConverter as cc # -> carla 시뮬레이터 화면에 대한 시각화, 그래픽처리 기능 제공.

import argparse # -> 명령행 인수를 사용할 수 있게 해줌.
import collections # -> namedtuple, deque, counter 등의 특별한 데이터 구조를 제공.
import datetime # -> 현재 날짜, 시간을 제공.
import logging # -> logging 기능을 제공. / logging -> 프로그램 실행중 발생하는 이벤트를 기록. ex) 디버그, 정보, 경고, 오류 등
import math # -> 수학함수, 상수를 제공.
import random # -> 난수를 제공.
import re # -> 정규 표현식 편집기능 제공. 문자열 검색, 추출, 치환 작업에 유용.
import weakref # -> 약한참조 제공

try:
    import pygame # -> 파이썬으로 게임, 멀티미디어 앱을 개발할 수 있게 하는 라이브러리
    from pygame.locals import KMOD_CTRL # -> 키보드 이벤트 관련 상수들
...
try:
    import numpy as np # -> 수치계산, 다차원 배열 처리 기능을 제공.
except ImportError:
